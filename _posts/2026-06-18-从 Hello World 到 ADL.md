...
5. 在 `std` 命名空间中，找到了 `std::ostream& operator<<(std::ostream&, const char*)` 函数，因此该 *非成员候选函数* 被加入候选列表。
6. 编译器对所有候选函数进行重载决议，发现 `std::ostream& operator<<(std::ostream&, const char*)` 满足最佳匹配规则，因此最终调用该函数。
